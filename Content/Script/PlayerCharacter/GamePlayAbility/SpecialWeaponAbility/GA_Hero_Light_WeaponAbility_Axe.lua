...
---@type GA_Hero_Light_WeaponAbility_Axe_C
local M = UnLua.Class()
...
function M:HandleApplyDamage(InPayload)
   self.CachedTargetActor = InPayload.Target
   local DamageEffectSpecHandle = self:MakeHeroDamageEffectSpecHandle(self.EffectClass,self:GetHeroCombatComponentFromActorInfo():GetHeroCurrentEquippedWeaponDamageAtLevel(self:GetAbilityLevel()),self.CurrentAttackTypeTag,4)
    if not DamageEffectSpecHandle then
        return
    end
    self:BP_ApplyEffectSpecHandleToTarget(self.CachedTargetActor, DamageEffectSpecHandle)
    local Context = UE.FGameplayEffectContextHandle()
    self:K2_ExecuteGameplayCue(self.GameplayCueTag, Context)
    UE.UAbilitySystemBlueprintLibrary.SendGameplayEventToActor(self.CachedTargetActor, self.EventTag1, InPayload)
end
